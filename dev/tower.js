module.exports = function () {
	for (const roomName in Game.rooms) {
		...
		const rangePointFlag = room.find(FIND_FLAGS, { filter: { color: COLOR_YELLOW } })[0]
		const rangePoint = rangePointFlag ? rangePointFlag.pos : spawn.pos
		...
		for (let t = 0; t < towers.length; t++) {
			...
			const hostile = _.sortBy(rangePoint.findInRange(FIND_HOSTILE_CREEPS, attackRange, {
			...
			let healCreep = rangePoint.findInRange(FIND_MY_CREEPS, healRange, {
			...
			let healGuard = rangePoint.findClosestByRange(FIND_MY_CREEPS, {
